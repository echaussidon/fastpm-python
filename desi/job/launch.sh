...
if [[ $fnl_0 == 'True' ]]; then
    for aout in ${aout_list[@]}; do
        ...
        power_spectrum=$(sbatch --job-name power_spectrum-ini-fnl-0-${aout}-${release}-${region}-${npasses} --parsable\
                         ...
                         power_spectrum.job $sim_name $aout $release $region $npasses $name_randoms 'True' 'False' 'False' 'False')
        echo "    * power_spectrum: "$power_spectrum
        
        power_spectrum=$(sbatch --job-name power_spectrum-inicorr-fnl-0-${aout}-${release}-${region}-${npasses} --parsable\
                         --dependency=afterok:$imaging_systematics\
...
if [[ $fnl_25 == 'True' ]]; then
    for aout in ${aout_list[@]}; do
        ...
        power_spectrum=$(sbatch --job-name power_spectrum-inicorr-fnl-25-${aout}-${release}-${region}-${npasses} --parsable\
                         --dependency=afterok:$imaging_systematics\
                         power_spectrum.job $sim_name $aout $release $region $npasses $name_randoms 'False' 'True' 'False' 'False')
        echo "    * power_spectrum: "$power_spectrum
...
if [[ $fnl_12 == 'True' ]]; then
    for aout in ${aout_list[@]}; do
        ...
        make_desi_survey=$(sbatch --job-name make_desi_survey-fnl-12-${aout}-${release}-${region}-${npasses} --parsable\
                           make_desi_survey.job $sim_name $aout $release $region $npasses $generate_randoms $name_randoms $generate_contamination $name_contamination $seed_data $seed_randoms)
        echo "    * make_desi_survey: "$make_desi_survey

        imaging_systematics=$(sbatch --job-name imaging_systematis-fnl-12-${aout}-${release}-${region}-${npasses} --parsable\
                              --dependency=afterok:$make_desi_survey\
                              imaging_systematics.job $sim_name $aout $release $region $npasses $name_randoms $which_contamination)
        echo "    * imaging_systematics: "$imaging_systematics

        power_spectrum=$(sbatch --job-name power_spectrum-ini-fnl-12-${aout}-${release}-${region}-${npasses} --parsable\
                         --dependency=afterok:$imaging_systematics\
                         power_spectrum.job $sim_name $aout $release $region $npasses $name_randoms 'False' 'True' 'False' 'False')
        echo "    * power_spectrum: "$power_spectrum
        
        power_spectrum=$(sbatch --job-name power_spectrum-inicorr-fnl-12-${aout}-${release}-${region}-${npasses} --parsable\
                         --dependency=afterok:$imaging_systematics\
                         power_spectrum.job $sim_name $aout $release $region $npasses $name_randoms 'False' 'True' 'False' 'False')
        echo "    * power_spectrum: "$power_spectrum

        power_spectrum=$(sbatch --job-name power_spectrum-cont-fnl-12-${aout}-${release}-${region}-${npasses} --parsable\
                         --dependency=afterok:$imaging_systematics\
                         power_spectrum.job $sim_name $aout $release $region $npasses $name_randoms 'False' 'False' 'True' 'False')
        echo "    * power_spectrum: "$power_spectrum

        power_spectrum=$(sbatch --job-name power_spectrum-corr-fnl-12-${aout}-${release}-${region}-${npasses} --parsable\
                         --dependency=afterok:$imaging_systematics\
                         power_spectrum.job $sim_name $aout $release $region $npasses $name_randoms 'False' 'False' 'False' 'True')
        echo "    * power_spectrum: "$power_spectrum
    done
