...
def build_angular_mask_for_contamination(tracer='QSO', region='SGC', data_dir='/global/u2/e/edmondc/Software/regressis/data', nside=256,
                                         expected_density=200, contamination_factor=1.5, seed=44):
    """ TODO """
    import os
    import fitsio
    import healpy as hp

    from mockfactory import HealpixAngularMask
    from regressis.footprint import DR9Footprint

    # load dr9 footprint
    convert_dict = {'N': 'north', 'DN': 'south_mid_ngc', 'N+SNGC': 'ngc', 'SNGC': 'south_mid_ngc', 'DS': 'south_mid_sgc', 'SSGC': 'south_mid_sgc', 'DES': 'des'}
    dr9 = DR9Footprint(nside, mask_lmc=False, clear_south=False, mask_around_des=False, cut_desi=False, verbose=False)(convert_dict[region])

    # healpix map at nside=256 with nested=True
    main_qso = np.load(os.path.join(data_dir, f'MAIN_{tracer}_{nside}.npy')) / hp.nside2pixarea(nside, degrees=True)
    fracarea = fitsio.FITS(os.path.join(data_dir, f'pixweight-dr9-{nside}.fits'))[1]['FRACAREA_12290'].read()
    # correct target density by the fracarea
    with np.errstate(divide='ignore', invalid='ignore'):
        main_qso = main_qso / fracarea
    main_qso[~dr9] = np.nan
    # Create map
    mean_density = np.nanmean(main_qso)
    main_qso /= mean_density
    # dowgrade it to nside=64 to homogenize it
    main_qso = hp.ud_grade(main_qso, 64, order_in='NESTED')

    # Build Healpix angular mask
    angular_mask = HealpixAngularMask(main_qso - (contamination_factor - 1), nest=True)

    return angular_mask, (contamination_factor * mean_density - expected_density)
